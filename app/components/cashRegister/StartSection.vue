...
async function openCashRegister() {
 ...
 try {
 const session = await $fetch<CashRegisterSession>('/api/cashRegister/open', {
 method: 'POST',
 body: {
...
<template>
 ...
 <span class="grid size-14 shrink-0 place-items-center rounded-2xl bg-[#f1f6fa] text-[#456a88]">
 ...
 <p class="text-xs uppercase tracking-[.16em] text-[#64748b]">Paso 1</p>
 <p class="mt-2 text-sm font-semibold">Cuenta el fondo</p>
 </div>
 <div class="rounded-2xl bg-[#f7fafc] p-4">
 <p class="text-xs uppercase tracking-[.16em] text-[#64748b]">Paso 2</p>
 <p class="mt-2 text-sm font-semibold">Registra caja inicial</p>
 </div>
 <div class="rounded-2xl bg-[#f7fafc] p-4">
 <p class="text-xs uppercase tracking-[.16em] text-[#64748b]">Paso 3</p>
 ...
 <p class="mt-1 text-sm text-[#64748b]">Solo puede existir una caja abierta a la vez.</p>
 </template>
 ...
 <div class="rounded-2xl bg-[#f7fafc] p-4">
 <p class="text-sm text-[#64748b]">Caja inicial</p>
 <p class="mt-1 text-3xl font-bold tracking-[-.04em] text-[#456a88]">{{ currency.format(cashSession.openingAmount) }}</p>
 <p class="mt-3 text-xs text-[#64748b]">Responsable: {{ cashSession.openedBy.fullName }}</p>
 <p class="text-xs text-[#64748b]">Inicio: {{ dateTime.format(new Date(cashSession.openedAt)) }}</p>
 </div>
 <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
 <UButton to="/dashboard/sales" block label="Ir a vender" icon="i-lucide-shopping-cart" />
 <UButton to="/dashboard/cashRegister?section=close" block label="Cerrar caja" icon="i-lucide-calculator" variant="soft" />
 </div>
 </div>
