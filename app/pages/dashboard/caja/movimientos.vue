...
import type { CashMovement, CashMovementPaginatedResponse, CashMovementType } from '~/types/cash-register'

definePageMeta({ middleware: 'auth' })
useHead({ title: 'Movimientos de caja' })
...
const { data, status, error, refresh } = useFetch<CashMovementPaginatedResponse>('/api/cash-movements', {
  query: { page, limit },
...
async function saveMovement() {
  ...
  formError.value = ''
  try {
    const movement = await $fetch<CashMovement>('/api/cash-movements', {
      method: 'POST',
      body: {
        type: form.type,
...
<template>
  <DashboardShell eyebrow="Caja" title="Movimientos de caja">
