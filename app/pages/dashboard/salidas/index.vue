...
import type { StockExit, StockExitReason } from '~/types/stock-exit'
...
async function registerExit() {
  ...
  formError.value = ''
  try {
    const stockExit = await $fetch<StockExit>('/api/stock-exits', {
      method: 'POST',
      body: {
        ...
        <template #header>
          <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p class="mt-1 text-sm text-[#78827c]">Selecciona un producto, registra la cantidad y deja una nota si hace falta.</p>
            </div>
            <UButton to="/dashboard/salidas/historial" label="Ver historial" icon="i-lucide-list" variant="soft" />
          </div>
        </template>

        <div class="space-y-5">
          <UFormField label="Buscar producto" name="product-search">
            <UInput v-model="search" icon="i-lucide-search" placeholder="Nombre o SKU" autocomplete="off" class="w-full" />
          </UFormField>

          <div v-if="isSearchingProducts" class="flex items-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm text-emerald-800" role="status" aria-live="polite">
          ...
          <div v-if="products.length" class="overflow-hidden rounded-2xl border border-[#e1e6e2] bg-white">
            <ul class="divide-y divide-[#edf0ed]" aria-label="Productos encontrados">
              <li v-for="product in products" :key="product.id">
                <button class="flex w-full items-center gap-3 p-4 text-left transition hover:bg-[#f7f9f7]" :class="selectedProduct?.id === product.id ? 'bg-[#edf4ef]' : ''" @click="selectProduct(product)">
                  <span class="grid size-10 shrink-0 place-items-center rounded-xl bg-[#eaf2ed] text-[#286047]"><UIcon name="i-lucide-package" class="size-5" /></span>
                  <span class="min-w-0 flex-1"><span class="block truncate text-sm font-semibold">{{ product.name }}</span><span class="mt-1 block text-xs text-[#7d8781]">{{ product.sku }} · {{ product.stock }} {{ product.unit === 'KILOGRAM' ? 'kg' : 'pzas' }}</span></span>
                  <span class="text-sm font-semibold">{{ currency.format(product.price) }}</span>
                </button>
              </li>
            </ul>
          </div>

          <form v-if="selectedProduct" class="space-y-4 rounded-2xl border border-[#e1e6e2] bg-[#fbfcfb] p-4" @submit.prevent="registerExit">
            <div>
              <p class="font-semibold">{{ selectedProduct.name }}</p>
              <p class="mt-1 text-xs text-[#7d8781]">Disponible: {{ selectedProduct.stock }} {{ selectedProduct.unit === 'KILOGRAM' ? 'kg' : 'pzas' }}</p>
            </div>
